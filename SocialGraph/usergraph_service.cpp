...
std::optional<Person> UserGraphService::find_person(int id)
{
...
std::vector<Person> UserGraphService::find_people(std::vector<int> ids)
{
...
std::vector<int> UserGraphService::shortest_path(int source_id, int dest_id)
{
    if (source_id < 0 || dest_id < 0) return {};

    if (source_id == dest_id) return { source_id };

    std::map<int, int> prev_node_keys = _shortest_path(source_id, dest_id);
    if (prev_node_keys.empty()) return {};

    // Iterate through the path_ids backwards, starting at dest_id
    std::vector<int> path = { dest_id };
    int prev_node_key = prev_node_keys[dest_id];
...
std::map<int, int> UserGraphService::_shortest_path(int source_id, int dest_id)
{
    // Use the id to get the Person
    auto source = find_person(source_id);
    if (!source.has_value()) return std::map<int, int>();
    ...
    std::map<int, int> prev_node_keys;
    ...
    while (!queue.empty()) {
        ...
        for (int friend_id : node.friend_ids) {
